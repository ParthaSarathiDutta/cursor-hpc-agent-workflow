...
- **Autonomous batch loop (NERSC):** UI Start submits a full Slurm chain once (`GPU afterany→Range`, `Range afterok→next GPU`); authoritative state in `<run_folder>/.agentic_loop/workflow.json`; Range runs `range_core` on Perlmutter (trials + sacct walltime + changemodel). See `docs/autonomous-iterative-loop.md`.
